...
- **Serviço B (Quarkus + gatilho nativo fino, ver ADR-006)** — `functions/`
  - `RelatorioAgendadoTrigger` (Timer Trigger, nativo, sem CDI) → `POST /internal/relatorio-agendado` (Quarkus) → `RelatorioService` (CDI, Panache): gera relatório periódico com médias, contagens por dia/urgência e a lista de avaliações (descrição, urgência, data de envio), e persiste.
  - `FeedbackCriticoTrigger` (Queue Trigger, nativo, sem CDI) → `POST /internal/feedback-critico` (Quarkus) → `EmailService` (CDI): envia e-mail aos admins com descrição, urgência e data de envio.
  - Exatamente 2 funções serverless (ADR-005), cada uma com gatilho nativo fino + endpoint interno Quarkus (ADR-006). Endpoints `/internal/*` protegidos por `InternalSecretValidator` (`X-Internal-Secret`).
- **Infraestrutura (`infra/azure/main.bicep`)**
  ...
  - Segredos (`postgres-admin-password`, `jwt-secret`, `internal-trigger-secret`) armazenados no Key Vault e referenciados nativamente pelos dois serviços, nunca em texto puro nas variáveis de ambiente.
...
# Resultado: BUILD SUCCESS — testes cobrem AgregadosJsonSerializer (unitário puro) e os 2
# endpoints internos via @QuarkusTest + RestAssured + Dev Services (Postgres via Testcontainers),
# incluindo rejeição com 401 sem/segredo incorreto e sucesso com o segredo certo
```

Ambiente usado para validar: Java 21 (Temurin/OpenJDK), Maven 3.8.7, Docker 29.6.1. O ambiente sandbox onde este projeto foi gerado tinha um Docker Engine muito recente (API 1.55); foi necessário fixar a versão do Testcontainers em 1.21.4 (nos dois módulos, inclusive sobrepondo a versão trazida pelo quarkus-bom) para a negociação de versão da API funcionar — deixe essa nota caso o mesmo sintoma ("client version X.XX is too old") apareça em outro ambiente.

## Checklist de produção

- [x] Build local (`mvnw`/`mvn`) dos dois serviços sem erros.
- [x] Testes unitários e de integração passando localmente (incluindo os 2 endpoints internos Quarkus).
...
- [ ] Validação end-to-end das 2 funções serverless contra Azure Storage Queue e Azure Communication Services reais, dentro do host real do Azure Functions — inclui confirmar que a chamada HTTP interna (gatilho nativo → endpoint Quarkus, via `WEBSITE_HOSTNAME`) funciona de verdade em produção, não só localmente com `localhost:7071`.
- [ ] Revisão de custo dos recursos provisionados por `infra/azure/main.bicep` (Action Group, 2 Metric Alerts, além dos recursos já previstos).
- [ ] Rotação do `JWT_SECRET`, `INTERNAL_TRIGGER_SECRET` e da senha do admin seed antes de qualquer deploy real.
...
- `.github/workflows/deploy-azure.yml`: dispara manualmente ou em push para `main`; faz login via OIDC (sem credencial de longa duração), builda e publica a imagem no ACR, atualiza o Container App, e builda/faz deploy do pacote de funções (Quarkus-augmented jar + gatilhos nativos) via `azure-functions-maven-plugin`. Depende de secrets do GitHub (`AZURE_CLIENT_ID`, `AZURE_TENANT_ID`, `AZURE_SUBSCRIPTION_ID`, `AZURE_ACR_NAME`, `AZURE_RESOURCE_GROUP`) e de um `environment` chamado `production` — nenhum desses foi configurado neste momento (ação humana, ver `docs/AZURE-DEPLOY.md`).
...
- As 2 funções do Serviço B nunca rodaram dentro do host real do Azure Functions (Core Tools/Azure) — só a lógica de negócio isolada (`RelatorioService`, `EmailService`, `AgregadosJsonSerializer`) foi validada localmente via `@QuarkusTest`. Em particular, a chamada HTTP function-to-function (gatilho nativo → endpoint interno via `WEBSITE_HOSTNAME`) nunca foi exercitada num Function App real.
